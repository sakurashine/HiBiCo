...
def knn_monitor(net, memory_data_loader, test_data_loader,
    ...
    print("len(val_loader.dataset.classes)",classes)  # 17
    total_top1, total_top5, total_num, feature_bank = 0.0, 0.0, 0, []
    feature_labels=[]
    avgpool = nn.AdaptiveAvgPool2d((1, 1))
    with torch.no_grad():
        for k,(data, target, _, _) in enumerate(memory_data_loader):
            ...
            print("KNN feature accumulation %d/%d"%(k,len(memory_data_loader)))  # 126/126
        # [D, N]
        torch.cuda.empty_cache()
        print("gpu consuming before combining:", torch.cuda.memory_allocated() / 1024 / 1024)
        # feature bank shape:  torch.Size([4002, 64])
        # feature label shape: torch.Size([4002])
        ...
        for  k,(data, target) in test_bar:
            ...
            # print("current eval feature shape: ",feature.shape)  # torch.Size([32, 64])
            print({'#KNN monitor Accuracy': total_top1 / total_num * 100})
    feature_bank.shape # torch.Size([4030, 64])
    feature_labels.shape # torch.Size([4030])
    del feature_bank
    del feature_labels
