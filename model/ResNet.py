...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, use_feature=True ) -> Tensor:
        ...
        x = x.cuda()  
        x = self.conv1(x)
        if use_feature and self.layer==1:
            return x
        x = self.bn1(x)
        # print("bn1", x.shape)  # torch.Size([32, 128, 15, 15])
        x = self.relu(x)
        # print("relu", x.shape)  # torch.Size([32, 128, 15, 15])
        x = self.maxpool(x)
        # print("maxpool", x.shape)  # torch.Size([32, 128, 15, 15])
        x15 = self.layer1[0].conv1(x)
        if use_feature and self.layer==15:
            return x15
        x = self.layer1(x)
        if use_feature and self.layer==2:
            return x
        # print("layer1", x.shape)  # torch.Size([32, 128, 15, 15])
        x25 = self.layer2[0].conv1(x)
        if use_feature and self.layer==25:
            return x25
        x = self.layer2(x)
        if use_feature and self.layer==3:
            return x
        # print("layer2", x.shape)  # torch.Size([32, 128, 8, 8])
        # x = self.layer3(x)
        # x = self.layer4(x) 
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
