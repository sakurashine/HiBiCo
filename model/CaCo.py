...
class CaCo(nn.Module):
    ...
    def __init__(self, base_encoder,args, dim=128, m=0.999, N_BANDS=1, LAYER=1):
        """
        ...
        self.args=args
        self.m = m
        self.encoder_q = base_encoder(num_classes=dim, num_bands=N_BANDS, layer=LAYER)  
        self.encoder_k = base_encoder(num_classes=dim, num_bands=N_BANDS, layer=LAYER)
    ...
    def forward_withoutpred_sym(self,im_q,im_k,moco_momentum):
        ...
        q_pred_upper = self.encoder_q(im_q, use_feature=True)
        q_pred_upper = nn.functional.normalize(q_pred_upper, dim=1)
        k_pred_upper = self.encoder_q(im_k, use_feature=True)  # queries: NxC
        k_pred_upper = nn.functional.normalize(k_pred_upper, dim=1)

        q_pred_upper = q_pred_upper.view(q_pred_upper.size(0), -1)
        k_pred_upper = k_pred_upper.view(k_pred_upper.size(0), -1)

        with torch.no_grad():  
            self._momentum_update_key_encoder_param(moco_momentum)# update the key encoder
            q_upper = self.encoder_k(im_q, use_feature=True)  # keys: NxC
            q_upper = nn.functional.normalize(q_upper, dim=1)
            q_upper = q_upper.view(q_upper.size(0), -1)
            # q_upper = conv1_MLP(q_upper)
            q_upper = q_upper.detach()
            
            k_upper = self.encoder_k(im_k, use_feature=True)  # keys: NxC
            k_upper = nn.functional.normalize(k_upper, dim=1)
            k_upper = k_upper.view(k_upper.size(0), -1)
            # k_upper = conv1_MLP(k_upper)
            k_upper = k_upper.detach()
